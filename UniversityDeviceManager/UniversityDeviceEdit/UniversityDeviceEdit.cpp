...
// UniversityDeviceEdit.cpp : 定义应用程序的类行为。
//
...
// 唯一的一个 CUniversityDeviceEditApp 对象
...
void exit_fn1(void)
{
	...
	if(g_bRestart)
	{
		...
		CString strPath = buf;
		USES_CONVERSION;
		WinExec(T2A(strPath), SW_SHOW);//启动strPath路径下的执行文件
	}
	return;
}
// CUniversityDeviceEditApp 初始化
...
BOOL CUniversityDeviceEditApp::InitInstance()
{
	...
	{
		AfxMessageBox(_T("只能启动一个程序，或者不能和查询软件同时启动！"));
		return FALSE;
	}
	atexit(exit_fn1);
